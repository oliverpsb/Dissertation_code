...
rolling_window = 6  # months per Sharpe calculation
anova_sharpe_results = {}

for factor in factor_cols:
    samples = []
    # Group data by regime and calculate monthly Sharpe ratios
    for regime, data in merged.groupby('Regime'):
        # Need at least rolling_window observations for a Sharpe
        if len(data) >= rolling_window:
            rolling_mean = data[factor].rolling(window=rolling_window, min_periods=rolling_window).mean()
            rolling_std = data[factor].rolling(window=rolling_window, min_periods=rolling_window).std()
            rolling_sharpes = (rolling_mean / rolling_std).dropna()
        else:
            rolling_sharpes = pd.Series(dtype=float) # empty if too short
        
        samples.append(rolling_sharpes.values)

        # Only keep regimes that have at least 2 Sharpe Values
        valid_samples = [s for s in samples if len(s) >= 2]

        if len(valid_samples) >=2:
            f_stat, p_value = f_oneway(*valid_samples)
            anova_sharpe_results[factor] = {'F-statistic': f_stat, 'p-value': p_value}
        else:
            anova_sharpe_results[factor] = {'F-statistic': None, 'p-value': None}


# Save ANOVA results to CSV
anova_sharpe_df = pd.DataFrame(anova_sharpe_results).T
anova_sharpe_df.to_csv(f'analysis_output/pc{n_comps}_r{num_reg}_anova_rolling_sharpe_results.csv')
logging.info(f'ANOVA on rolling Sharpe ratios saved as pc{n_comps}_r{num_reg}_anova_rolling_sharpe_results.csv')
